...
public class RideServiceImpl implements RideService {
	...
	public RideServiceImpl(RideRepository rideRepository, RideEntityDtoMapper rideMapper,
			HaversineService haversineService, TakenRideService takenRideService,
	...
	public List<RideDto> search(RideRequestDto rideRequest) {
		return rideRepository.findAll().stream()
				.filter(ride -> haversineService.isOnWay(ride, rideRequest)
	...
	private boolean isLaterToday(LocalDateTime rideTime, LocalDateTime rideRequestTime) {
		return rideTime.getYear() == rideRequestTime.getYear() && rideTime.getMonth() == rideRequestTime.getMonth()
	...
	public List<RideDto> findUpcomingByUserId(Long userId) {
		return rideRepository.findAll().stream()
				.filter(ride -> ride.getDriver().getId() == userId && ride.getDateTime().isAfter(LocalDateTime.now()))
				.map(ride -> rideMapper.toDto(ride)).collect(Collectors.toList());
	}
	...
	public List<RideDto> findUpcomingTakenRidesByUserId(Long userId) {
		...
		return rideRepository.findAll().stream().filter(ride -> rideIds.contains(ride.getId()))
				.map(ride -> rideMapper.toDto(ride)).collect(Collectors.toList());
	}
